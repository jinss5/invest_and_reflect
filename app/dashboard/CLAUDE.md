# CLAUDE.md

### Journal entry form sections

The dashboard renders a scrollable form with four card sections:

1. **Basic Info** — date picker, one-line summary
2. **News** — dynamic add/remove news items (headline + market reaction), overall interpretation textarea
3. **Market** — sentiment segmented control (bullish/neutral/bearish), Fear & Greed slider (0-100), market notes textarea
4. **Actions** — dynamic add/remove action items, each with: type dropdown (buy/sell/hold), ticker, shares, price/unit, confidence level, decision basis; followed by shared reasoning textarea
